...
export function initPongGame() {
    ...
    loader.load('./assets/source/inside_galaxy_skybox/scene.gltf', function (gltf) {
    ...
    loader.load('./assets/source/sci-fi_spaceship_bridge/scene.gltf', function (gltf) {
        scene.add(gltf.scene);
        const model = gltf.scene;
        ...
        // Rotate the model 90 degrees on the X, Y, or Z axis
        // model.rotation.y = -Math.PI / 2; // Rotate 90 degrees on the Y axis
        model.position.set(0, 85, -200);

    }, undefined, function (error) {
        console.error(error);
    });

    loader.load('./assets/source/transport_shuttle/scene.gltf', function (gltf) {
        // scene.add(gltf.scene);
        const model = gltf.scene;

        // Set the size by scaling the model
        model.scale.set(27, 27, 27); // Adjust the values to resize (x, y, z)
    ...
    loader.load('./assets/source/wall_of_the_spaceship/scene.gltf', function (gltf) {
                    ...
                    if (Array.isArray(child.material)) {
                        ...
                        child.material.forEach((mat, index) => {
                            if (mat.color) {
                                ...
                                console.log(`Material ${index} does not have a color property`);
                            }
                        });
                    } else {
    ...
    const floorMaterial = new THREE.MeshStandardMaterial({
        color: new THREE.Color(0.06353375545877023, 0.17683924545207097, 0.2438378681299503)
    }); // Grey material
    ...
    loader.load('./assets/source/bmo_adventure_time/scene.gltf', function (gltf) {
        console.log("***************************************");
        const model = gltf.scene;
